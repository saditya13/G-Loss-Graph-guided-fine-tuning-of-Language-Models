...
We now include a detailed breakdown of training time in Figure 5 of the appendix. Empirically, G-Loss introduces no measurable computational overhead from graph construction, normalization, or matrix inversion, as all operations are confined to mini-batch scale. The average time per epoch and total training time for both G-Loss and baseline methods are reported in Table 3, showing that G-Loss achieves comparable or slightly lower wall-clock training time due to faster convergence.
