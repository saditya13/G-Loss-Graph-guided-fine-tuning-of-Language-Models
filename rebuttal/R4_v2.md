...
We agree that the visual differences in Figure 4 (t-SNE plots) between G-Loss+CE and SCL/CE for the MR and R8 datasets are not highly prominent. In the camera-ready version, we plan to remove the existing one and include embedding visualizations from the standalone training pipelines for these datasets, which offer a clearer view of how G-Loss structures the embedding space independently of the classifier.

For datasets such as R52, 20NG and Ohsumed, which contain a larger number of classes, the 2D t-SNE projection tends to produce overlapping regions that are difficult to interpret visually. Therefore, for clarity, we will focus on MR and R8, where the effects can be meaningfully illustrated. A sample plot comparing SCL and G-Loss embeddings on the R8 dataset (from the standalone mode) is attached for reference, showing more distinct inter-class separation under G-Loss. 


*Instead of relying on visual observations, can we resort to using some clustering score to validate the claim?*

Yes, To quantify representation quality, we report the Silhouette coefficient for G-Loss and other baseline losses across all datasets in Table 3, where G-Loss improves average Silhouette scores by 15.34 percent over SCL (as shown above), 12.46 percent over CE and 15.08% over triplet, confirming more compact intra-class clusters and greater inter-class separation.
...
We sincerely thank you for advising extend the scope of G-Loss formulation beyond text classification tasks. Its design—centered on graph-guided relational consistency is inherently modality-agnostic, making it suitable for vision–language. Currently, going so would shift the paper’s primary focus from language modeling to vision-based tasks. We therefore plan to explore this direction in future work by evaluating G-Loss with vision–language models such as CLIP. 

Nevertheless, we agree to include results on larger language models (>1B parameters)—for example, DeBERTaV3-1.1B under quantized settings—in the camera-ready version. Based on feedback from other reviewers, we have also begun evaluating G-Loss on larger datasets, and following that, we will extend the experiments to larger models and include the results in our final experimental pipeline.
